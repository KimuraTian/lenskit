...
/**
 * fsSLIM model build context provider which using item-item similarity to find item neighbors in order to train SLIM model
 *
 ...
 */
public class FsSLIMBuildContextProvider implements Provider<SLIMBuildContext> {
    private static final Logger logger = LoggerFactory.getLogger(FsSLIMBuildContextProvider.class);
    ...
    @Inject
    public FsSLIMBuildContextProvider(@Transient RatingVectorPDAO dao,
                                      @Transient UserVectorNormalizer normlzr,
                                      @Transient ItemItemModel simModel) {
        this.rvDAO = dao;
        normalizer = normlzr;
        itemSimilarityModel = simModel;
     ...
     */
    private void buildItemNeighbors(Long2ObjectMap<LongSortedSet> itemNeighbors) {
        LongIterator iter = itemSimilarityModel.getItemUniverse().iterator();
        while (iter.hasNext()) {
